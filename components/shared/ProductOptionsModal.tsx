...
export default function ProductOptionsModal({ product, onClose }: ProductOptionsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      {/* 3D Neo-Brutalist Modal Box */}
      <div className="relative w-full max-w-[550px] bg-white border-2 border-[#1A0500] rounded-[32px] p-6 md:p-8 flex flex-col z-10 shadow-[8px_8px_0px_#1A0500]">
        
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-5 right-5 w-9 h-9 rounded-full bg-[#FFF5F5] hover:bg-[#FFD1D1] border border-[#FF8A8A] flex items-center justify-center text-[#1A0500] transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex-1 overflow-y-auto max-h-[50vh] pr-1 flex flex-col gap-6">
              ...
              {(['Mild', 'Medium', 'Hot', 'Extra Spicy'] as const).map((level) => {
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2.5 rounded-full border-2 font-sans font-semibold text-xs md:text-sm text-center transition-all duration-150 cursor-pointer select-none ${
                      isSelected
                        ? 'bg-[#E63900] border-[#1A0500] text-white shadow-[2px_2px_0px_#1A0500]'
                        : 'bg-white border-[#FF8A8A] text-[#1A0500] hover:border-[#1A0500]'
                    }`}
                  >
                    {level}
                  </button>
                ...
                return (
                  ...
                  >
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                      isChecked ? 'bg-[#E63900] border-[#1A0500]' : 'border-[#FF8A8A] bg-white'
                    }`}>
          ...
          <div className="flex flex-col items-start w-full">
            ...
            <textarea
              ...
              className="w-full h-24 px-4 py-3 bg-white border border-[#FF8A8A] rounded-[20px] shadow-[4px_4px_0px_#FF8A8A] text-black font-sans text-sm placeholder:text-[#B0B0B0] focus:outline-none focus:border-[#E63900] transition-colors resize-none"
            />
          </div>
        ...
        {/* Footer Actions Row */}
        <div className="mt-8 pt-5 border-t border-[#1A0500]/10 flex items-center justify-between gap-4">
          
          {/* Quantity Selector Counter */}
          <div className="flex items-center gap-3.5 bg-[#FFF5F5] border-2 border-[#1A0500] rounded-full px-4 py-1.5 shadow-[2px_2px_0px_#FF8A8A]">
            <button
